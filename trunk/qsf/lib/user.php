		...
		if(isset($this->cookie[$this->sets['cookie_prefix'] . 'user']) && isset($this->cookie[$this->sets['cookie_prefix'] . 'pass'])) {
			...
			$user = $this->db->fetch("SELECT m.*, s.skin_dir, g.group_perms, g.group_name, z.*
				...
				WHERE m.user_id=%d AND
				      ...
				      g.group_id=m.user_group AND
				      z.zone_id=m.user_timezone LIMIT 1", $cookie_user, $cookie_pass);
			...
			$user = $this->db->fetch("SELECT m.*, s.skin_dir, g.group_perms, g.group_name, z.*
				...
				WHERE m.user_id=%d AND
				      ...
				      s.skin_dir=m.user_skin AND
				      g.group_id=m.user_group AND
				      z.zone_id=m.user_timezone LIMIT 1", $session_user, $this->ip, $session_pass);

		}else {
			$user = $this->db->fetch("SELECT m.*, s.skin_dir, g.group_perms, g.group_name, z.*
				FROM %pusers m, %pskins s, %pgroups g, %ptimezones z
				WHERE m.user_id=%d AND
				      s.skin_dir=m.user_skin AND
				      g.group_id=m.user_group AND
				      z.zone_id=m.user_timezone LIMIT 1", USER_GUEST_UID);
