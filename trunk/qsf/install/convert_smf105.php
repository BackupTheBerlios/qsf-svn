...
else if( $_GET['action'] == 'posts' )
{
   ...
   while( $row = $oldboard->db->nqfetch($result) )
   {
      ...
      $qsf->db->query( "INSERT INTO {$qsf->pre}posts VALUES( {$row['ID_MSG']}, {$row['ID_TOPIC']}, '{$row['ID_MEMBER']}', '{$row['smiliesEnabled']}', 1, '{$row['body']}', {$row['posterTime']}, '', '{$row['posterIP']}', '', '{$row['modifiedTime']}' )" );
      $i++;
   }
   if( $i == $all )
   {
