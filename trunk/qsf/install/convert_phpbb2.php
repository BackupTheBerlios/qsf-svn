...
if( $_GET['action'] == '' )
{
   ...
   include 'templates/convert_phpbb_memberprofiles.php';
...
else if( $_GET['action'] == 'posts' )
{
   ...
   while( $row = $oldboard->db->nqfetch($result) )
   {
      ...
      $qsf->db->query( "INSERT INTO {$qsf->pre}posts VALUES( {$row['post_id']}, {$row['topic_id']}, '{$row['poster_id']}', {$row['enable_smilies']}, {$row['enable_bbcode']}, '{$message}', {$row['post_time']}, '', '{$ip}', '', 0 )" );
      $i++;
   }
   if( $i == $all )
   {
