   ...
   while( $row = $oldboard->db->nqfetch($result) )
   {
      if( $row['id'] > 0 )
      {
         ...
         $qsf->db->query( "INSERT INTO %pusers
            ...
            VALUES( %d, '%s', %d, '%s', %d, '%s', '%s', %d, %d, '%s', %d, '%s', '%s', %d, '%s', %d, '%s', '%s', '%s', '%s', '%s', %d, %d, %d, %d, %d )",
            $row['id'], $row['name'], $row['legacy_password'], $row['joined'], $row['title'], $row['mgroup'], $avatar, $type, $width, $height, $row['email'], $showmail, $bday, $row['website'], $row['posts'], $row['location'], $row['icq_number'], $row['msnname'], $row['aim_name'], $row['yahoo'], $row['interests'], $row['signature'], $row['last_visit'], $row['last_activity'], $row['email_pm'], $row['view_sigs'], $row['view_avs'] );
         $i++;
      }
   }
