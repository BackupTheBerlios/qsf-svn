 ...
 *
 * This convertor has been tested on unmodified databases for MyBB 1.0 PR2
 **/
...
define('CONVERTER_NAME', 'MyBB 1.0 PR2 Convertor');
...
if( !isset($_GET['action']) || $_GET['action'] == '' )
{
   ...
   include 'templates/convert_mybb_memberprofiles.php';
...
else if( $_GET['action'] == 'topics' )
{
   ...
   while( $row = $oldboard->db->nqfetch($result) )
   {
      ...
      $row['subject'] = strip_mybb_tags( $row['subject'] );
      $qsf->db->query( "INSERT INTO {$qsf->pre}topics VALUES( {$row['tid']}, {$row['fid']}, '{$row['subject']}', '', {$row['uid']}, '{$row['uid']}', '', '{$row['dateline']}', {$row['replies']}, {$row['views']}, {$topic_modes}, 0, '' )" );
      $i++;
   }
